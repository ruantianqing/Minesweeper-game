...
var mineSweepingMap = function (r, c, num) {
    ...
    var writeInMine = function (num) {
        ...
        var randomLocation = function () {
            var x = Math.floor(Math.random() * r)   // 随机生成雷的坐标
            var y = Math.floor(Math.random() * c)
            // console.log( ':', x, y);
            if (map[x][y] !== 9) {  // 数字9代表雷，如果此坐标不是雷，就设成类
                map[x][y] = 9
            } else {
                randomLocation()    // 是雷的话就重新随机位置
            }
        }
        for (var i = 0; i < num; i++) {
            randomLocation()    // 循环num次，埋下num个雷
        }
    }
...
// 4，清除画面，然后写入新的画面，初始化游戏
var stopTime
var initializeGame = function (row, col, num) {
    var residue = document.querySelector('.residue')    
    residue.innerText = `${num}`                        // 剩余雷数
    var time = document.querySelector('.tick')
    time.innerText = `0`                                // 游戏计时
    var i = 1
    clearInterval(stopTime)
    stopTime = setInterval(function () {                // 每过一秒钟时间加一秒
        time.innerText = `${i++}`
    }, 1000)
    // zz
    zz = 0
    ...
    box.innerHTML = ''                              // 清除原地图
    var body = document.querySelector('body')
    body.style.minWidth = `${27 * col}px`           // 设置最小宽度
    var map = mineSweepingMap(row, col, num)        // 生成新的地图
    writeHtml(map)                                  // 将地雷写入页面中
    clearMine(row, col, num)                        // 开始扫雷
}
